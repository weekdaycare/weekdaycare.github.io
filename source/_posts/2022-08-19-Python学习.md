...
{% meting "1895813372" "netease" "song" "autoplay" "mutex:false" "listmaxheight:340px" "preload:none" "theme:#4169e1"%}
...
#这里给一下类似于C的解释，比较好懂
#C:for(i=1;i<n;i++)
start即为i的初始值(i = start)
stop即为i的结束值(不包含stop)(n = stop)
step即为i步径的跨度(step = i+=)
```

<div class="warning">

> 不论使用哪种`range`，它的参数只能是整型

</div>

<details>
<summary>栗子时间到</summary>
...
可以使用`append()`，功能是在列表的**末尾**来添加一个指定的元素
